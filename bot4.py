...
class Alien:
    ...
    alien_id = 0
    def __init__(self, grid, bot_pos, k, indi=None):
        self.grid = grid
        indices = self.grid.get_unoccupied_open_indices()
        ind = rd.choice(indices)
        x_upper = min(bot_pos[0] + k, 34)
        x_lower = max(bot_pos[0] - k, 0)
        y_upper = min(bot_pos[1] + k, 34)
        y_lower = max(bot_pos[1] - k, 0)

        while x_lower <= ind[0] <= x_upper and y_lower <= ind[1] <= y_upper:
            ind = rd.choice(indices)
...
class bot4:
    def __init__(self, grid, alpha = 0.1, k=2, debug=1, bot_pos=None):
        ...
        while self.pos == self.grid.crew_pos or self.pos == self.grid.crew_pos2 or self.pos is None:
            self.pos = rd.choice(self.grid._grid.get_open_indices()) if bot_pos == None else bot_pos
        self.debug=debug
        if self.debug:
            print(self.pos)
        self.alpha = alpha
...
def main():
    ...
    for k in range(2, 12, 2):
        ...
        print(f"For k: {k}")
        print(f"Bot 4 success list: {bot4_steps}")
        print()

        bot1_avg = sum(bot4_steps) / 200
