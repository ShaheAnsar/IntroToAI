...
class bot6:
    ...
    def find_upper_and_lower(self, grid_x, grid_y):
        ...
        upper_x = min(self.D, self.pos[0] + self.k)
    ...
    def diffuse_alien_prob(self, alien_found):
        ...
        if alien_found:
            s = sum([self.grid.grid[j][i].alien_belief for j in range(self.D) \
                     for i in range(self.D) if self.grid.grid[j][i].open == True])
            (l_x, u_x), (l_y, u_y) = self.find_upper_and_lower(self.pos[0], self.pos[1])
            m_cells = [(i, j) for j in range(l_x, u_x + 1) for i in range(l_y, u_y + 1)\
            ...
            for j in range(self.D):
                for i in range(self.D):
                    if self.grid.grid[j][i].open == False:
                        continue

                    # TODO: check if the passed coordinates are correct
                    if self.within_alien_sensor((i, j)):
                        self.grid.grid[j][i].alien_belief *= alpha
                    else:
                        self.grid.grid[j][i].alien_belief *= (1 / alpha)
